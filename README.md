...
An admin ability to remove/delete an organization outright (suspend is reversible; delete isn't, so it's intentionally left out). On the project board: drag-and-drop (status changes via a dropdown for now), editing a task after creation (delete-and-recreate for now), and project-level membership/permissions beyond "any org member can do anything" — see the original product spec for the full roadmap.
