...
- **Tests + CI** — `npm test` runs the [Vitest](https://vitest.dev) suite (`src/**/*.test.ts`): pure unit tests for `lib/plans.ts`/`lib/admin.ts`, and integration tests for `lib/api-keys.ts`/`lib/invitations.ts` that hit a real Postgres via Prisma. [.github/workflows/ci.yml](.github/workflows/ci.yml) runs build, lint, and the test suite (against a Postgres service container) on every push to `main` and every pull request.
